...
One arg constructor which creates a new B+ Tree with the value given as the max number of keys for each node.
*/
BPlusTree::BPlusTree(int maxKeys) {
    maxKeysLeaf = maxKeys;
...
char* BPlusTree::getCurrentInstructions() {
    return instructions;
}

// Search
/*
BPlusTreeNode* BPlusTree::find(int key)

Recursive wrapper for the find(key root) function. This will return a node pointer to the node that contains the value.
*/
BPlusTreeNode* BPlusTree::find(int key) {
    clearInstructions();
...
BPlusTreeNode* BPlusTree::find(int key, BPlusTreeNode* currentNode) {
    ...
    snprintf(instructions + strlen(instructions), sizeof(instructions) - sizeof(strlen(instructions)), "Node found.\n");
    snprintf(instructions + strlen(instructions), sizeof(instructions) - sizeof(strlen(instructions)), "----------\n");
    snprintf(instructions + strlen(instructions), sizeof(instructions) - sizeof(strlen(instructions)), "Find Complete\n");
    snprintf(instructions + strlen(instructions), sizeof(instructions) - sizeof(strlen(instructions)), "----------\n\n");
    return currentNode;
}
...
int BPlusTree::insert(int key) {
    ...
    // Since the current node is out of room.
    // Split the current node. After the split, check to ensure the parent doesn't need split.
    ...
    while(currentNode->getKeysLength() >= maxKeysLeaf) {
        ...
        if(currentNode == root) {
            ...
            snprintf(instructions + strlen(instructions), sizeof(instructions) - sizeof(strlen(instructions)), "Update the parent to include the newly created node to the right in its children.\n");
        }
...
int BPlusTree::remove(int key) {
    ...
    /* If after the while loop finishes, the current node is the root and the root only has one child, the child becomes the root and the old root is removed. */
...
// Helpers
/*
void BPlusTree::printNode(BPlusTreeNode* currentNode)
...
queue<BPlusTreeNode*> BPlusTree::treeToQueue() {
...
void BPlusTree::clearInstructions() {
